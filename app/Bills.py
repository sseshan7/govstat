...
class Bills:
    ...
    def load_bills_into_mysql(cls, last_mod_flag=False):
        ...
        for bill_type in BillType.types:
            db_bill_type = getattr(
                BillType, bill_type.upper()
            )  # db_bill_types starts counting at 1
            ...
            for dir_name in os.listdir(type_path):
                ...
                xmldata = ET.parse(
                    os.path.join(type_path, dir_name, XML_FILE)
                ).getroot()
                ...
                if last_mod_flag:
                    ...
                    try:
                        with open(
                            os.path.join(type_path, dir_name, LAST_MOD_FILE1), "r"
                        ) as f:
                            last_mod = f.read()
                    except IOError:
                        with open(
                            os.path.join(type_path, dir_name, LAST_MOD_FILE2), "r"
                        ) as f:
                            last_mod = f.read()
                    ...
                    if (today - last_mod).days > 1:
                        ...
                        if not bills:
                            ...
                            Bills.fully_populate_bill(
                                jsondata, xmldata, num, db_bill_type
                            )
                        else:
    ...
    def return_sql_json_by_date(cls, date):
        ...
        for bill_type_abbr in BillType.types:
            bill_type = getattr(
                BillType, bill_type_abbr.upper()
            )  # bill_types starts counting at 1
