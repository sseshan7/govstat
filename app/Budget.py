...
key_index_dict["phys_res_perc_outlays"] = [
    39,
    "Physical Resources Percentage of Outlays",
]
...
key_index_dict["other_funcs_perc_outlays"] = [
    41,
    "Other Functions Percentage of Outlays",
]
...
def read_mysql_deficit_surplus():
    ...
    tot_data_query = db.session.query(DeficitSurplus.total_receipt).order_by(
        DeficitSurplus.year
    )
    data_dict["total_receipts"] = [x[0] for x in tot_data_query.all()]
    tot_data_query = db.session.query(DeficitSurplus.total_outlay).order_by(
        DeficitSurplus.year
    )
    data_dict["total_outlays"] = [x[0] for x in tot_data_query.all()]
    tot_data_query = db.session.query(DeficitSurplus.total_net).order_by(
        DeficitSurplus.year
    )
...
def read_mysql_receipt_breakdown():
    ...
    data_query = db.session.query(ReceiptBreakdown.indiv_income_tax).order_by(
        ReceiptBreakdown.year
    )
    data_dict["indiv_income_tax"] = [x[0] for x in data_query.all()]
    data_query = db.session.query(ReceiptBreakdown.corp_income_tax).order_by(
        ReceiptBreakdown.year
    )
    data_dict["corp_income_tax"] = [x[0] for x in data_query.all()]
    data_query = db.session.query(ReceiptBreakdown.soc_ins_retire_total).order_by(
        ReceiptBreakdown.year
    )
    data_dict["insurance_retirement"] = [x[0] for x in data_query.all()]
    data_query = db.session.query(ReceiptBreakdown.excise_tax).order_by(
        ReceiptBreakdown.year
    )
    data_dict["excise_tax"] = [x[0] for x in data_query.all()]
    data_query = db.session.query(ReceiptBreakdown.other).order_by(
        ReceiptBreakdown.year
    )
...
def read_mysql_outlay_breakdown():
    ...
    for k in key_index_dict:
        data_query = db.session.query(getattr(OutlayBreakdown, k)).order_by(
            OutlayBreakdown.year
        )
